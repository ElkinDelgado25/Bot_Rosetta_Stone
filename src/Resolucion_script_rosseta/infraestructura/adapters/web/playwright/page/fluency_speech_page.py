...
from Resolucion_script_rosseta.aplicacion.ports.fluency_speech import FluencySpeechPort
...
(() => {
  ...
  window.__rosettaStartMicNoise = () => {
    ...
    asegurarContexto().then(() => {
      ...
      if (window.__rosettaMicCheckBuffer) {
        ...
        fuente.loop = true;
        volumen.gain.value = 1.75;
        fuente.connect(volumen);
        volumen.connect(state.bus);
        fuente.start();
        window.__rosettaMicNoise = { fuentes: [fuente], volumen, modo: "file" };
        return;
      }

      window.__rosettaMicCheckMode = "tone";
      const volumen = state.context.createGain();
      const filtro = state.context.createBiquadFilter();
      const fuentes = [220, 440, 660].map((frecuencia) => {
      ...
      filtro.Q.value = 0.9;
      volumen.gain.value = 0.85;
      filtro.connect(volumen);
      volumen.connect(state.bus);
      for (const osc of fuentes) osc.start();
      window.__rosettaMicNoise = { fuentes, volumen, modo: "tone" };
  ...
  window.__rosettaStopMicNoise = () => {
    ...
    for (const fuente of activo.fuentes || []) {
      try { fuente.stop(); } catch (e) {}
  ...
  window.__rosettaMicCheckState = () => {
    ...
    return {
      ...
      modo: window.__rosettaMicCheckMode,
...
class PlaywrightFluencySpeechPage(FluencySpeechPort, LoggingMixin):
    ...
    async def _speak_until_accepted(self, audio: bytes, step_number: int) -> bool:
        ...
        for intento in range(1, self.speech_attempts + 1):
            ...
            # El ``onended`` del audio inyectado es una buena señal, pero no la
            # única. En algunas conversaciones B1 el reproductor queda atado a
            # su propia reproducción un poco más de la cuenta y esta espera
            # larga terminaba matando la actividad aunque el reconocimiento ya
            # estuviera listo. Si no avisa a tiempo, se continúa: el veredicto
            # real sigue siendo el del botón de enviar.
            try:
                await self._wait(
                ...
                self.logger.debug(
                    "  El audio inyectado no avisó de terminar; se continúa"
                )
    ...
    async def _dump_screenshot(self, description: str) -> None:
        try:
            from Resolucion_script_rosseta.infraestructura.core import get_base_dir
    ...
    async def _load_mic_check_audio(self) -> None:
        ...
        carpetas = (base_dir / "audio", base_dir / "data" / "audio")
        for nombre in ("mic_check.wav", "mic_check.mp3", "mic_check.m4a", "mic_check.ogg"):
            for carpeta in carpetas:
                ...
                try:
                    ...
                    self.logger.info("  No se pudo cargar %s: %s", archivo, error)
                return
    ...
    async def _dismiss_microphone_check(self) -> bool:
        ...
        try:
            ...
            if estado is not None:
                self.logger.info(
                    "  Comprobación de micrófono en pantalla (%s, %s/%s, modo=%s): %s",
                    "escuchando" if estado.get("escuchando") else "eligiendo micrófono",
                    estado.get("encendidas"),
                    estado.get("barras"),
                    estado.get("modo") or "desconocido",
                    estado.get("texto"),
                )
                await self.page.evaluate(
